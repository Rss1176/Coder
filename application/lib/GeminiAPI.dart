  ...
  Future<Map<String, dynamic>> generateQuestions(String abilityLevel, String codingLanguage) async{
    ...
    try {
      final questionResponse = await http.post( // async function using the http package that goes to the API URL and awaits questions
        Uri.parse(endpoint + '?key=$apiKey'), // combines the end point with the api key creating the link to the AI account, the URI.parse calls to the https address
      ...
      if (questionResponse.statusCode == 200) { // enssure http has connected properly with a response
        ...
        String explanation = lines.sublist(6).join(' '); //everyline after 6 is the explination, combines all the lines after 6 into a list before joining them together as a string with a whitespace seperator
        return {
